<script setup lang="ts">
    ...
    const visibleLayers = computed(() => [matter, antimatter].filter(l => unref(l.treeNode.visibility) === Visibility.Visible))
</script>

<template>
    <div>
        <template v-for="(l, i) of visibleLayers" :key="l.id">
            <span :style="'color: ' + l.color">{{format(Decimal.log10(unref(l.points)))}}</span>
